...
#export HF_HOME="/data/wangzihao/.cache/huggingface"

# *[Specify the config file path]
export OMINI_CONFIG="train/config/premier_user_linear.yaml"
...
# 参数设置
MIN_FREE_MEMORY=50000   # 默认显存阈值40000
GPU_NUM=1
...
get_gpus_by_memory() {
    ...
    while IFS=, read -r index memory_free; do
        ...
        # 检查显存是否满足条件
        if [[ "$memory_free" =~ ^[0-9]+$ ]] && (( memory_free >= min_memory )); then
            gpu_ids+=("$index")
            # 找到足够数量的GPU即停止
...
execute_func() {
  accelerate launch --main_process_port 10810 -m omini.train_flux.train_user_embedding_linear
