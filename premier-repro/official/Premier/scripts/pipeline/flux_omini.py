...
class Condition(object):
    def __init__(
            ...
            ori_condition: Union[Image.Image, torch.Tensor] = None,
            adapter_text_setting: Union[str, dict] = None,
            position_delta=None,
            position_scale=1.0,
            latent_mask=None,
            is_complement=False,
        ...
        self.ori_condition = ori_condition
        self.adapter = adapter_setting
        self.text_adapter = adapter_text_setting
...
def attn_forward(
    ...
    for i, hidden_state in enumerate(hidden_states2):
        with specify_lora((attn.to_add_out,), adapters[i + h2_n]):
...
def generate(
    ...
    # Prepare conditions
    c_latents, uc_latents, c_ids, c_timesteps, c_text, c_text_ids = ([], [], [], [], [], [])
    c_projections, c_guidances, c_adapters, c_text_adapters = ([], [], [], [])
    ...
    for condition in conditions:
        ...
        if redux_pipeline is not None:
            if condition.ori_condition is not None:
                text_tokens_c, text_ids_c = condition.encode_redux_delta(redux_pipeline)
            else:
                text_tokens_c, text_ids_c = condition.encode_redux(redux_pipeline)
            c_text.append(text_tokens_c)
            c_text_ids.append(text_ids_c)
            c_text_adapters.append(condition.text_adapter)
    ...
    if redux_pipeline is not None:
        # Disable the attention cross different condition branches
        if model_config.get("text_redux", True):
            group_mask[1: 1 + len(conditions), 1: 1 + len(conditions)] = torch.diag(
                torch.tensor([1] * len(conditions)))
        else:
            group_mask[: 1 + len(conditions), : 1 + len(conditions)] = torch.diag(
                torch.tensor([1] * (len(conditions) + 1)))
        group_mask[2 + len(conditions):, 2 + len(conditions):] = torch.diag(torch.tensor([1] * len(conditions)))
        group_mask[1:1 + len(conditions), 2 + len(conditions):] = False
        group_mask[2 + len(conditions):, 1:1 + len(conditions)] = False
        if model_config.get("redux_cross", False):
            group_mask[1 + len(conditions), 1:1 + len(conditions)] = True
        ...
        for i, t in enumerate(timesteps):
            ...
            if redux_pipeline is not None:
                noise_pred = transformer_forward(
                    ...
                    text_features=[prompt_embeds] + (c_text if redux_pipeline is not None else []),
                    img_ids=[latent_image_ids] + (c_ids if use_cond else []),
                    txt_ids=[text_ids] + (c_text_ids if redux_pipeline is not None else []),
                    timesteps=[timestep] * (2 + len(conditions)) + (c_timesteps if use_cond else []),
                    pooled_projections=[pooled_prompt_embeds] * (2 + len(conditions))
                                       + (c_projections if use_cond else []),
                    guidances=[guidance] * (2 + len(conditions)) + (c_guidances if use_cond else []),
                    return_dict=False,
                    adapters=[main_adapter] + (c_text_adapters if redux_pipeline is not None else [])
                             + [main_adapter] + (c_adapters if use_cond else []),
                    cache_mode=mode if kv_cache else None,
                    cache_storage=kv_cond if kv_cache else None,
                    to_cache=[False, *[True] * len(c_latents), False, *[True] * len(c_latents)],
