...
> 画像を5回選ぶと、AIがあなたの好みを言葉にします。好みを確かめて、同じお題の描き分けとおすすめを見てみましょう。

見せたいものは「選択 → 好みの解釈 → 描き方 → おすすめ」のつながりである。性格診断にはせず、色・光・構図・質感などの美的嗜好に限定する。
...
- 9/19確認の実機: RTX 4090、VRAM 24,564 MiB、ホストRAM約62 GiB。確認時は別プロセスがVRAM約9 GiBを使用していた。展示起動前に所有者を確認して停止する運用とし、本調査では停止していない。
...
**1回に1sessionだけ受け付ける。** persona確認中のLLM保持もこの前提で許容する。二重送信はjob IDで抑止し、別端末の並行GPU要求は受け付けない。既存の常駐LLMを無断で共有・停止する実装にしない。
