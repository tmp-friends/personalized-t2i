...
作成日: 2026-09-19 / 展示日: 2026-09-23 / 状態: 設計案・実装前

添付提案を起点に、ZIPP・PIGRewardの論文と公開モデルを確認して作成した設計書。[FAN中心の旧展示案](2026-09-19-personalized-t2i-exhibition-demo-design.md)を更新する。今回モデルの取得・推論・速度測定は行っていない。以下の性能値は設計目標である。
...
| FLUXが必要 | 必須ではない。既存資産を使えるSDXLから検証 |
...
添付提案の自由入力例は、まず「東京の夜景」等を固定お題として実現する。自由入力は品質と待ち時間の検証後に追加する。
...
色・光・構図・質感／描画表現・雰囲気を最大5項目で表示する。根拠のない項目は「まだ分かりません」とする。性格・年齢・職業を推定しない。

各項目から根拠になった選択画像を確認できる。「今回は反映しない」と少数の訂正候補を用意する。確かさは「複数の選択で見られた傾向／1回だけの仮説」と根拠数で示し、校正されていない確率は出さない。

### 5.3 お題を選び、描く

初期お題は「窓辺の猫」「東京の夜景」「森の小さな家」「海辺の灯台」「雨の街角」「夜のカフェ」の6件。日本語表示と検証済み英語の基本プロンプトを対応付ける。
...
基本プロンプトの被写体、数、動作、場所、時間帯、明示的な色指定を守る。「赤い猫」を寒色嗜好だけで青い猫へ変えない。好みは元のお題と矛盾しない配色・光・質感・構図へ適用する。
...
英語プロンプトは使用するSDXLの両tokenizerで切り詰めが発生しない長さへ収める。語数だけで判定しない。persona欄や解説文をそのまま生成器へ流し込まない。長過ぎる／意味保持に失敗した出力は1回だけ修正し、再失敗時は通常側のキャッシュを表示し、「今回は好みを反映できませんでした」と案内する。この実行では個人化生成・PIGReward推薦を省略する。

固定6お題の不変条件は設定ファイルに記載し、初日の実例とリハーサルで被写体・数・場所が保たれるか確認する。

### 7.2 生成器

第一候補は `stabilityai/stable-diffusion-xl-base-1.0` のbase単独、fp16、追加学習なし。base単独での利用は公式に案内されている。[SDXLモデルカード](https://huggingface.co/stabilityai/stable-diffusion-xl-base-1.0)

初期検証設定は1024×1024、20 steps、guidance 7.0、negative promptなし、batch size 1。schedulerとrevisionは初日の試験結果から固定する。4枚を同時batchにすることは前提にしない。遅延が予算を超える場合は768×768も測定し、品質を確認して切り替える。設定変更時は通常側キャッシュも再生成する。

既存のSDXL loader・生成テストは参考資産であり、今回の8枚比較やワーカー切替の確認済みを意味しない。FANのencoder差し替えを使わない通常SDXL経路を用意する。

基本構成の動作確認後、時間に余裕があればIllustrious系も生成器の候補として試し、画質・嗜好の反映・生成時間を比較する。
...
UIはGradio、調停はPythonの単一orchestratorとする。ワーカーは別プロセスにし、UIが重いモデルを直接importしない。ローカルのJSON入出力でモデル依存を分け、外部公開サービスの構成はMVPに持ち込まない。
...
| 画像生成 | SDXLのみ | 4枚とmetadataをCPU側へ保存し、プロセス終了 |
...
5. 4090単体で採用reasoner（準備時のみ）、LLM、SDXL、evaluatorを順次起動し、残留VRAMが次の処理を妨げないことを確認する。外部VLMを準備時だけ使う場合は、現地でreasonerを起動する必要がないことも確認する。
...
| 日付 | 作るもの | 完了条件 |
|---|---|---|
| 9/19 | PIGReward入出力、1対の根拠、1比較、SDXL4枚の計測 | 不確かなモデル接続を試し、失敗内容も残す |
...
本書は展示の実装範囲と判定条件を定める設計案。実機での成立確認と実装計画の作成は次の作業とする。
